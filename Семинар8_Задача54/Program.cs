...
{
    int[] row = new int[n]; // объявляем временный одномерный массив row с количеством элементов равным
    // количеству элементов в каждой строке исходного массива.
    for (int i = 0; i < m; i++)
    {
        for (int j = 0; j < n; j++)
        row[j] = matrix[i, j];
        Array.Sort(row); // применяем метод сортировки Sort() одномерного массива
        for (int k = n-1; k >= 0; k--) //перебор массива row начмнаем с последнего элемента массива для сортировки по убыванию
        {
        matrix[i, k] = row[k];
